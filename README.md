...
await db.update("User", { age: 35, isActive: false }, { email: "alice@example.com" });
```
...
const users = await db.searchFields("User", { isActive: true }, ["username", "email"]);
```
...
const result = await db.searchPaginated(
  "User",
  { isActive: true },
  2, // page number
  10 // results per page
